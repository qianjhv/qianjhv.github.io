...
如果你感兴趣的话，非常欢迎探讨，虽然是静态博客，但也会集成第三方工具实现评论功能。
...
2. @next/mdx 配置 - 项目根目录下 `next.config.ts`。（在没有使用 @next/mdx 的项目中，需要手动在 `webpack.config.js` 中配置 @mdx-js/loader）
...
- 完成后就 可以正确渲染，如 app/test/page.mdx 可以在浏览器中通过根域名 /test 访问（这样显然有些麻烦，我们希望使用一个文件夹统一管理 markdown 文件）
...
由于后续步骤需要 css 样式化，在这里我们先使用 [next-themes](https://github.com/pacocoursey/next-themes) 设置全局的亮色主题和暗色主题，以方便后续操作。
